...
export async function getNaiveBayesResult(req, res) {
  try {
    ...
    const logYesComponents = [`log(P(YES)) = ${Math.log(asdYESPrior).toFixed(4)}`];
    const logNoComponents = [`log(P(NO)) = ${Math.log(asdNoPrior).toFixed(4)}`];

    for (let i = 1; i <= 10; i++) {
      const key = `a${i}`;
      const isYes = patientData.answers[key] === "YES";
      const yesLikelihood = isYes ? likelihoodYesAsdYes[`A${i}`] : likelihoodNoAsdYes[`A${i}`];
      const noLikelihood = isYes ? likelihoodYesAsdNo[`A${i}`] : likelihoodNoAsdNo[`A${i}`];
      logYes += Math.log(yesLikelihood);
      logNo += Math.log(noLikelihood);
      logYesComponents.push(
        `log(P(${key}=${isYes ? "YES" : "NO"} | YES)) = ${Math.log(yesLikelihood).toFixed(4)}`
      );
      logNoComponents.push(
        `log(P(${key}=${isYes ? "YES" : "NO"} | NO)) = ${Math.log(noLikelihood).toFixed(4)}`
      );
    }
      ...
      data: {
        ...
        calculationDetails: {
          ...
          posterior: {
            ...
            normalization: `P(YES) = exp(${logYes.toFixed(4)}) / (exp(${logYes.toFixed(4)}) + exp(${logNo.toFixed(4)}))`,
          },
        },
      },
    });
